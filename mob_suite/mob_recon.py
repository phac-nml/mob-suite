...
def main():
    ...
    replicon_contigs = getRepliconContigs(replicon_blast(replicon_ref, fixed_fasta, 90, 90, args.evalue, tmp_dir,replicon_blast_results,num_threads=num_threads))

    logging.info('Running relaxase blast on {}'.format(mob_ref))
    mob_contigs = getRepliconContigs(mob_blast(mob_ref, fixed_fasta, 90, 90, args.evalue, tmp_dir,mob_blast_results,num_threads=num_threads ))

    logging.info('Running contig blast on {}'.format(plasmid_ref_db))
    contig_blast(fixed_fasta, plasmid_ref_db, args.min_ident, args.min_cov, args.evalue, 1000,
    ...
    repetitive_contigs = repetitive_blast(fixed_fasta, repetitive_mask_file, 85, 75, args.evalue, 300, tmp_dir, repetitive_blast_results,num_threads=num_threads)
    ...
    for contig_id in contig_seqs:
        if contig_id not in filter_list:
            ...
            if contig_id in circular_contigs:
                contig_status = 'Circular'
            results_fh.write("{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\n".format(file_id, 'chromosome', contig_id, len(contig_seqs[contig_id]),
                                                                  contig_status, '',
                                                                  '', '', '', '',
                                                                  '', rep_dna_info))
    results_fh.close()
