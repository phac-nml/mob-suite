...
def parse_args():
    ...
    parser = ArgumentParser(description='Mob Suite: Typing and reconstruction of plasmids from draft and complete assemblies')
    ...
    parser.add_argument('--min_length', type=str, required=False, help='Minimum length of contigs to classify', default=1000)
    parser.add_argument('--min_ident', type=str, required=False, help='Minimum sequence identity', default=90)
    ...
    parser.add_argument('--unicycler_contigs', type=str, required=False, help='Check for circularity flag generated by unicycler in fasta headers', default=True)
    parser.add_argument('--run_circlator', type=str, required=False, help='Run circlator minums2 pipeline to check for circular contigs',default=True)
...
def run_mob_typer(fasta_path,outdir,num_threads=1):
    mob_typer_path = os.path.join(os.getcwd(),'mob_typer.py')
...
def contig_blast(input_fasta, plasmid_db, min_ident, min_cov, evalue, min_length, tmp_dir,blast_results_file,num_threads=1,word_size=11):
    blast_runner = None
...
def main():
    ...
    replicon_contigs = getRepliconContigs(replicon_blast(replicon_ref, fixed_fasta, min_ident, min_cov, evalue, tmp_dir,replicon_blast_results,num_threads=num_threads))

    logging.info('Running relaxase blast on {}'.format(mob_ref))
    mob_contigs = getRepliconContigs(mob_blast(mob_ref, fixed_fasta, min_ident, min_cov, evalue, tmp_dir,mob_blast_results,num_threads=num_threads ))
    ...
    repetitive_contigs = repetitive_blast(fixed_fasta, repetitive_mask_file, min_ident, min_cov, evalue, 300, tmp_dir, repetitive_blast_results,num_threads=num_threads)
        ...
        if not contig_id in pcl_clusters:
            if contig_id in contig_seqs:
                if not clust_id in seq_clusters:
                    ...
                    pcl_clusters[contig_id] = "Novel_" + str(clust_id)
    ...
    for id in seq_clusters:
        ...
        for contig_id in cluster:
            if contig_id in circular_contigs and len(cluster) > 1:
    ...
    for cluster in seq_clusters:
        ...
        if count_rep == count_seqs or (float(count_rep) / count_seqs * 100 > 50 and count_small == count_seqs):
            continue
        ...
        for contig_id in clusters:
            ...
            results_fh.write("{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\n".format(file_id, cluster, contig_id, len(clusters[contig_id]),
                                                                  contig_status, found_replicon_string,
                                                                  found_replicon_id_string, found_mob_string, found_mob_id_string, mash_top_hit['top_hit'],
    ...
    for contig_id in contig_seqs:
        if contig_id not in filter_list:
            ...
            if contig_id in circular_contigs:
                contig_status = 'Circular'
            results_fh.write("{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\n".format(file_id, 'chromosome', contig_id, len(contig_seqs[contig_id]),contig_status, '','', '', '', '','', rep_dna_info))
    results_fh.close()
